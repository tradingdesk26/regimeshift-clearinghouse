...
/// @title InterAgentRepoV4
/// @notice V4 — addresses audit round-2 MED finding #2 (pause-to-default DOS).
///
/// Changes vs V3:
///  - repay() no longer gated by `whenNotPaused`. Pause now blocks ENTRY
///    (originate, liquidate, defaultLoan) but NOT EXIT (repay). This prevents
///    an owner-or-compromised-key actor from pausing right before expiry and
///    forcing the borrower into default (where they lose 4% via bounty +
///    insurance carve-outs that they would not pay during normal repayment).
///  - EIP-712 domain bumped to ("InterAgentRepo", "4") — V3 quotes can't replay
///
/// V3 already addressed audit round-1 HIGH findings #1-#4 + LOW #8-#9.
/// V4 inherits all those fixes verbatim.
///
/// R2-#1 (Chainlink staleness DoS in defaultLoan) deliberately NOT patched —
/// behavior aligned with Aave V3 / Compound III / Morpho Blue (all revert on
/// stale price). See audit/round2.md.
///
/// Audit references:
///   - audit/round1.md  (V3 fixes)
///   - audit/round2.md  (V4 fixes + acknowledged risks)
contract InterAgentRepoV4 is EIP712, ReentrancyGuard, Ownable, Pausable {
    ...
    // R2-#2 fix: NO `whenNotPaused` here. Pause must block entry (originate),
    // not exit (repay). Otherwise an admin pause right before expiry forces
    // borrower into default, losing 4% to carve-outs they wouldn't pay
    // during normal repayment. Industry standard (Aave V3 / Compound III /
    // Morpho Blue) leaves repay paths always open.
